...
function ProductCard({ p, devisHref }: { p: ProductModel; devisHref: string }) {
    ...
    return (
        <div className={`relative flex flex-col rounded-2xl border bg-white dark:bg-slate-800 transition-shadow hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,.12)] dark:hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,.4)] ${
            ...
            <div className="flex flex-col flex-1 p-6">
                ...
                {/* Nom + description */}
                <h3 className="text-base font-bold text-gray-900 dark:text-slate-100 leading-snug mb-1">
                    {p.name}
                </h3>
                {p.shortDescription && (
...
export default async function BoutiquePage({
    ...
    const grouped = CATEGORY_ORDER.reduce<Record<ProductCategory, ProductModel[]>>(
        (acc, cat) => {
            acc[cat] = allPublished.filter(p => p.category === cat);
            return acc;
        },
        {} as Record<ProductCategory, ProductModel[]>,
    );
